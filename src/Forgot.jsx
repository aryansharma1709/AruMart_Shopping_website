...
export default function Forgot() {
  function callApi(values) {
    console.log("data sending to api " + values.email1 );
  }
    ...
    <div className="bg-gradient-to-r from-primary-light via-primary-default to-primary-light h-screen flex flex-col justify-center items-center p-4">
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row shadow-lg bg-white w-full md:w-2/3 lg:w-1/2 h-auto md:h-120 justify-center items-center gap-4 p-4">
        ...
        <div className="flex flex-col items-center gap-2 md:gap-4 w-full md:w-1/2 p-4">
          ...
          <input onBlur={handleBlur} autoComplete="email1" name="email1" value={values.email1} onChange={handleChange} type="email" id="email" placeholder="Email address" className="border-2 border-gray-500 p-2 w-full" />
          {touched.email1 && errors.email1 && <div className=" font-thin text-primary-dark text-xs self-end">{errors.email1}</div>}
          <button type="submit" className="bg-primary-default text-white rounded-md px-8 py-2 w-full md:w-auto disabled:bg-primary-light" disabled={!isValid} >Send OTP</button>
